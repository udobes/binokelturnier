...
$spielerProGruppe = 3;
$turnierdatum = date("d.M.Y");
$turniername = "Preisbinokel Heroldstatt";
$ort = "Heroldstatt";
$beschreibung = "Vreanstalter: AKS-Heroldstatt";
...
//Paramter übertragen ins array
$param['turnierdatum']  = $turnierdatum;      
$param['turniername']   = $turniername  ;     
$param['teilnehmer']    = $anzahlSpieler ;    
$param['runden']        = $anzahlRunden   ;   
$param['spielergruppe'] = $spielerProGruppe;  
$param['ort']           = $ort              ; 
$param['beschreibung']  = $beschreibung      ;

if ($anzahlSpieler/$spielerProGruppe != floor($anzahlSpieler/$spielerProGruppe)) {
	echo "<h2>Die Anzahl Spieler muss durch $spielerProGruppe teilbar sein!<br>
		Bitte neuen Wert eingeben.</h2><p>";
	exit;
}
...
function turnier_speichern($name,$turnier,$param) {
	$fp = fopen("turniere/".$name.".trn", "w");
	fputs($fp,serialize($turnier));
	fclose($fp);
	$fp = fopen("turniere/".$name.".prm", "w");
	fputs($fp,serialize($param));
	fclose($fp);
}
...
turnier_speichern(date("YmdHis")."_Turnier",$turnier,$param);
